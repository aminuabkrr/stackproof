...
export function useCounter_SubmitProof() {
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      ...
      const fn = (contracts as any).counter_submitProof;
...
export function useCounter_GetProof() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).counter_getProof;
...
export function useCounter_GetProofCount() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).counter_getProofCount;
