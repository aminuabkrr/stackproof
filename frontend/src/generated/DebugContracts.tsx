...
import { useCounter_SubmitProof } from './hooks';



import { useCounter_GetProof } from './hooks';



import { useCounter_GetProofCount } from './hooks';
...
export default function DebugContracts() {
      ...
      <div style={S.panel}>
        ...
        {openContract === "counter" && (
          <div style={Object.assign({ padding: '20px', width:'100%' })} className='space-y-4'>
                ...
                {activeTab === 'write' && (
                  <FunctionCard_Counter_SubmitProof 
                    num="00"
                    isOpen={openFunction === "counter_submit-proof"}
                    onToggle={() => setOpenFunction(openFunction === "counter_submit-proof" ? null : "counter_submit-proof")}
                  />
                )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Counter_GetProof 
      num="01"
      isOpen={openFunction === "counter_get-proof"}
      onToggle={() => setOpenFunction(openFunction === "counter_get-proof" ? null : "counter_get-proof")}
    />
  )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Counter_GetProofCount 
      num="02"
      isOpen={openFunction === "counter_get-proof-count"}
      onToggle={() => setOpenFunction(openFunction === "counter_get-proof-count" ? null : "counter_get-proof-count")}
    />
  )}
...
function FunctionCard_Counter_SubmitProof({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useCounter_SubmitProof();
  const isReadOnly = false;

  
  const [args, setArgs] = useState<Record<string, string>>({
    'title': '', 'category': '', 'proof-url': '', 
  });
    ...
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          ...
          <span style={S.fnName}>SUBMIT-PROOF</span>
        </div>
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['title'] ?? '', {"string-ascii":{"length":100}}, 'title'),toClarityValue(args['category'] ?? '', {"string-ascii":{"length":50}}, 'category'),toClarityValue(args['proof-url'] ?? '', {"string-ascii":{"length":200}}, 'proof-url'),]); }}>
            ...
            <div>
              <label style={S.argLabel}>category <span style={S.argType}>STRING-ASCII</span></label>
              
              <input style={S.argInput} value={args['category']} onChange={e => setArg('category', e.target.value)} />
              
            </div>
            
            <div>
              <label style={S.argLabel}>proof-url <span style={S.argType}>STRING-ASCII</span></label>
              
              <input style={S.argInput} value={args['proof-url']} onChange={e => setArg('proof-url', e.target.value)} />
...
function FunctionCard_Counter_GetProof({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useCounter_GetProof();
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>GET-PROOF</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['proof-id'] ?? '', "uint128", 'proof-id'),]); }}>
          
            
            <div>
              <label style={S.argLabel}>proof-id <span style={S.argType}>UINT128</span></label>
              
              <input style={S.argInput} value={args['proof-id']} onChange={e => setArg('proof-id', e.target.value)} />
...
function FunctionCard_Counter_GetProofCount({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useCounter_GetProofCount();
  const isReadOnly = true;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>GET-PROOF-COUNT</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
